...
token_T* lexer_next_token(lexer_T* lexer) {
    while (lexer->c != '\0') {
        ...
        switch (lexer->c) {
            case '=':
                {
                    if (lexer_peek(lexer, 1) == '>') {
                        ...
                        return lexer_advance_with(
                                lexer, init_token("=", TOKEN_EQUALS));
                    }
                } break;
            case '(': return lexer_advance_current(lexer, TOKEN_LPRAEN);
            case ')': return lexer_advance_current(lexer, TOKEN_RPRAEN);
            ...
            case '\0': break;
            default: printf("[Lexer]: Unexpected character '\%d'\n'", lexer->c);
        }
    }
