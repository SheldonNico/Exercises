...
class Card {
  ...
  public:
    Card(CardRank r = RANK_ACE, CardSuit s = SUIT_CLUB) :
      m_rank {r}, m_suit {s} { }
...
class Deck {
  ...
  public:
    ...
    const Card& dealCard() {
      assert( m_cardIndex < static_cast<int>(m_deck.size()) );
    ...
    void shuffleDeck() {
      ...
      for (int index = 0; index < deck_size; ++index)
      {
        ...
        int swapIndex = getRandomNumber(0, deck_size - 1);
...
int main() {
  ...
  std::cout << "Game start! Shfffle card!\n";
