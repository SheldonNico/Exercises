...
#[derive(Debug, Default, Clone)]
pub struct P22RectSet {
    inner: Vec<P22Rect>
}

impl P22RectSet {
    fn on(&mut self, area: P22Area) {
        let mut rect = P22Rect { inner: area, neg: vec![] };
        for old in self.inner.iter() {
            rect.sub(old);
        }
        self.inner.push(rect);
    }

    fn off(&mut self, area: P22Area) {
        let rect = P22Rect { inner: area, neg: vec![] };
        for old in self.inner.iter_mut() {
            old.sub(&rect);
        }
    }

    fn count(&self) -> i32 {
        self.inner.iter().map(|r| r.count()).sum::<i32>()
    }
}
...
impl P22Rect {
    ...
    fn __intersect((nx, ny, nz): &P22Area, (sx, sy, sz): &P22Area) -> P22Area {
        (Self::_intersect(nx, sx), Self::_intersect(ny, sy), Self::_intersect(nz, sz))
    }

    fn intersect(&self, other: &Self) -> Self {
        let inner = Self::__intersect(&self.inner, &other.inner);
        let mut rect = P22Rect { inner, neg: vec![] };
        for old in self.neg.iter().chain(other.neg.iter()) {
            if rect.count() == 0 { return rect; } // avoid stack overflow
            rect.neg.push(old.intersect(&rect));
            // neg.push(old.intersect(&rect));
        }
        rect
    }
    ...
    fn sub(&mut self, other: &Self) {
        ...
        self.neg.push(intersect);
    }

    fn area((xs, ys, zs): &P22Area) -> i32 {
        (xs.len() * ys.len() * zs.len()) as i32
    }

    fn count(&self) -> i32 {
        let mut sum = Self::area(&self.inner);
        for rect in self.neg.iter() {
            sum -= rect.count();
        }
        sum as i32
    }
